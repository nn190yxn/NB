...
test('migrateMysql creates the collection and user doc tables', async () => {
  const statements = []
  const pool = { execute: async sql => { statements.push(sql) } }
  await migrateMysql(pool)
  assert.equal(statements.length, 3)
  assert.match(statements[0], /CREATE TABLE IF NOT EXISTS app_state/)
  assert.match(statements[1], /CREATE TABLE IF NOT EXISTS ip_collections[\s\S]*PRIMARY KEY \(collection, item_id\)/)
  assert.match(statements[2], /CREATE TABLE IF NOT EXISTS ip_user_docs[\s\S]*PRIMARY KEY \(user_id, doc_key\)/)
})
...
test('saveCollections upserts rows per item and prunes removed ids', async () => {
  ...
  assert.match(upserts[0][0], /ON DUPLICATE KEY UPDATE owner_id = VALUES\(owner_id\), data = VALUES\(data\)/)
  const prune = calls.at(-1)
  assert.match(prune[0], /DELETE FROM ip_collections WHERE collection = \? AND item_id NOT IN/)
  assert.deepEqual(prune[1], ['structures', '1', '2'])
})
...
test('saveCollections clears the collection when it becomes empty', async () => {
  ...
  assert.equal(calls.at(-1)[0], 'DELETE FROM ip_collections WHERE collection = ?')
  assert.deepEqual(calls.at(-1)[1], ['structures'])
})
