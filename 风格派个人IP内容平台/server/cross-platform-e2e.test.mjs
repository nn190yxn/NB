...
test('热点到素材、创作、发布和表现快照保持统一来源链路', async t => {
  ...
  assert.ok(material.source_refs.some(ref => ref.type === 'research' && ref.id === research.id))
  ...
  assert.ok(topics[0].source_refs.some(ref => ref.type === 'material' && ref.id === material.id))
  ...
  let draft = drafts.find(item => item.platform === '抖音')
  assert.ok(draft.source_refs.some(ref => ref.type === 'material' && ref.id === material.id))
  draft = await (await fetch(`${base}/api/drafts/${draft.id}/hooks`, { method: 'POST', headers, body: '{}' })).json()
  draft = await (await fetch(`${base}/api/drafts/${draft.id}`, { method: 'PUT', headers, body: JSON.stringify({ selected_hook_id: draft.hooks[0].id, version: draft.version }) })).json()
  draft = await (await fetch(`${base}/api/drafts/${draft.id}`, { method: 'PUT', headers, body: JSON.stringify({ fact_check_status: 'verified', version: draft.version }) })).json()
  draft = await (await fetch(`${base}/api/drafts/${draft.id}/checks/persona`, { method: 'POST', headers, body: '{}' })).json()
  draft = await (await fetch(`${base}/api/drafts/${draft.id}/checks/quality`, { method: 'POST', headers, body: '{}' })).json()
  draft = await (await fetch(`${base}/api/drafts/${draft.id}/checks/publish`, { method: 'POST', headers, body: '{}' })).json()
  const approval = await (await fetch(`${base}/api/drafts/${draft.id}/approve`, { method: 'POST', headers, body: JSON.stringify({ version: draft.version }) })).json()
  assert.equal(approval.draft.workflow_status, 'ready_to_shoot')
  assert.deepEqual(approval.draft.source_refs, draft.source_refs)
  assert.deepEqual(approval.shooting.source_refs, draft.source_refs)
  ...
  assert.deepEqual((await published.json()).source_refs, draft.source_refs)
